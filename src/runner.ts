...
/**
 * Run all iterations of an eval (separate API call for each run)
 */
async function runEval(
  client: Anthropic,
  evalDef: EvalDefinition,
  config: ExperimentConfig,
  skill: Awaited<ReturnType<typeof loadSkill>> | null,
  baseDir: string,
  ...
  // Prepare prompt once (with or without skill)
  ...
  // Run each iteration with its own API call
  for (let i = 1; i <= config.runs; i++) {
    ...
    try {
      ...
      // Parse and write generated files for this run
      ...
      if (writtenFiles.length === 0) {
        ...
        // Run tests on this run's generated files
      ...
      // API call or setup failed for this run
      const result: RunResult = {
        runNumber: i,
        passed: false,
        testsPassed: 0,
